...
def build_tvdi_for_month(target_date_or_month: str) -> pd.DataFrame:
    """
    Mongo -> pivot month x ten_xa -> interpolate(time)
    -> lấy row tháng target -> return df(ten_xa, tvdi, _key)
    """
    target_month = _normalize_month_str(target_date_or_month)
    ...
    row = pivot_interp.loc[target_ts]
    out = row.reset_index()
    out.columns = ["ten_xa", "tvdi"]
...
def get_hcm_wards_geojson_tvdi(date_or_month: str) -> dict:
    """
    Trả GeoJSON wards_hcm đã join tvdi theo tháng.
    properties: _key, tvdi
    """
    _load_boundaries()
    wards = _WARDS.copy()
    _, key_col = _META
    ...
    wards = wards.merge(df_tvdi[[key_col, "tvdi"]], on=key_col, how="left")
    wards = wards[[key_col, "tvdi", "geometry"]]
