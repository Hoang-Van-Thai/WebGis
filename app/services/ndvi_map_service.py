...
# ✅ data1 = ranh giới phường/xã TP.HCM
WARDS_SHP = os.path.join(DATA_DIR, "wards_hcm.shp")

# ------- cache shapefile để không load lại mỗi request -------
_WARDS = None
_META = None  # (ward_name_col, key_col)
...
def build_ndvi_for_date(target_date: str) -> pd.DataFrame:
    """
    Mongo -> pivot date x ten_xa -> interpolate(time)
    -> lấy row ngày target_date -> return df(ten_xa, ndvi, _key)
    """
    t = pd.to_datetime(target_date).normalize()
...
def get_hcm_wards_geojson_ndvi(date_str: str) -> dict:
    """
    Trả GeoJSON wards_hcm đã join ndvi theo date_str.
    properties: _key, ndvi
    """
    _load_boundaries()
    wards = _WARDS.copy()
    ...
    wards = wards.merge(df_ndvi[[key_col, "ndvi"]], on=key_col, how="left")
    wards = wards[[key_col, "ndvi", "geometry"]]
